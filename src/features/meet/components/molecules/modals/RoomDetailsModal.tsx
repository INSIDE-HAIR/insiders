...
const convertRoomToModalData = (
  ...
  return {
    roomInfo: {
      ...
      meetingLink:
        room.meetingUri ||
        `https://meet.google.com/${room.meetingCode || spaceId}`,
      startDate: room.startDate,
      endDate: room.endDate,
      accessType: {
        type: (room.config?.accessType || "OPEN").toLowerCase(),
        label:
          room.config?.accessType === "RESTRICTED"
            ? "Restringido"
            : room.config?.accessType === "TRUSTED"
              ? "De confianza"
              : "Abierto",
        className:
          room.config?.accessType === "RESTRICTED"
      ...
      status: {
        ...
        label: room.activeConference?.conferenceRecord
          ? "Conferencia Activa"
          : "Inactiva",
    ...
    members: {
      ...
      totalMembers:
        membersStats?.total || room._analytics?.permanentMembers?.total || 0,
    ...
    sessions: {
      summary: {
        ...
        averageParticipants:
          room._analytics?.sessions?.averageParticipantsPerSession || 0,
      },
...
const GeneralSectionWrapper = lazy(() =>
  Promise.resolve({
    default: ({
      navigation,
      roomData,
      ...
      <GeneralSectionDemo
        ...
        isUpdatingEndDate={isUpdatingEndDate}
      />
    ),
  })
);

const ReferencesSectionWrapper = lazy(() =>
  Promise.resolve({
    default: ({
      navigation,
      roomData,
      ...
      <ReferencesSectionDemo
        ...
        onTagRemove={(tagName) => {
          ...
          const tag = roomData.references.tags.assigned.find(
        ...
        onTagAdd={(tagName) => {
          ...
          // Find tag by name and call real function
          const tag = roomData.references.tags.available.find(
            (t: any) => t.name === tagName
          );
          if (tag) {
        ...
        onGroupRemove={(groupName) => {
          ...
          const group = roomData.references.groups.assigned.find(
        ...
        onGroupAdd={(groupName) => {
          ...
          // Find group by name and call real function
          const group = roomData.references.groups.available.find(
            (g: any) => g.name === groupName
          );
          if (group) {
...
const MembersSectionWrapper = lazy(() =>
  Promise.resolve({
    default: ({
      navigation,
      roomData,
...
const ConfigurationSectionWrapper = lazy(() =>
  Promise.resolve({
    default: ({
      navigation,
      roomData,
...
const SessionsSectionWrapper = lazy(() => {
  ...
  return Promise.resolve({
    default: ({ navigation, roomData, room, toast }: any) => {
      ...
      console.log(
        "🎬 SessionsSectionWrapper: Component function called with spaceId:",
        spaceId
      );
      return (
        <SessionsSectionDemo
...
const StatisticsSectionWrapper = lazy(() => {
  ...
  return Promise.resolve({
    default: ({ navigation, roomData, room }: any) => {
      const spaceId = room?.name?.split("/").pop() || null;
      console.log(
        "📊 StatisticsSectionWrapper: Component function called with spaceId:",
        spaceId
      );
      return <StatisticsSectionDemo spaceId={spaceId} />;
    },
  });
});
...
export const RoomDetailsModal: React.FC<RoomDetailsModalProps> = ({
    ...
    try {
      ...
      try {
        ...
        if (tagsResponse.ok && groupsResponse.ok) {
          ...
          console.log(
            "❌ Una o ambas APIs fallaron, continuando con mock data..."
          );
        }
      } catch (apiError) {
      ...
      console.log(
        "🎨 Ejecutando fallback a datos mock con paleta de colores vibrantes"
      );
      ...
      setAvailableTags([
        {
          id: "66c1f2e3b4a8c9d012345601",
          name: "Marketing",
          slug: "marketing",
          color: "#E91E63",
        },
        {
          id: "66c1f2e3b4a8c9d012345602",
          name: "Training",
          slug: "training",
          color: "#2196F3",
        },
        {
          id: "66c1f2e3b4a8c9d012345603",
          name: "Meeting",
          slug: "meeting",
          color: "#4CAF50",
        },
        {
          id: "66c1f2e3b4a8c9d012345604",
          name: "Estrategia",
          slug: "strategy",
          color: "#9C27B0",
        },
        {
          id: "66c1f2e3b4a8c9d012345605",
          name: "Desarrollo",
          slug: "development",
          color: "#FF9800",
        },
        {
          id: "66c1f2e3b4a8c9d012345606",
          name: "Ventas",
          slug: "sales",
          color: "#009688",
        },
        {
          id: "66c1f2e3b4a8c9d012345607",
          name: "Análisis",
          slug: "analysis",
          color: "#3F51B5",
        },
      ...
      console.log(
        "📝 Configurando assignedTags (simulando algunos ya asignados)..."
      );
      ...
      console.log(
        "📝 Configurando assignedGroups (simulando algunos ya asignados)..."
      );
      setAssignedGroups([
        {
          id: "66c1f2e3b4a8c9d012345703",
  ...
  const fetchConfigurationsData = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          `Error al cargar configuraciones: ${response.status} ${response.statusText}`
        );
      }
      ...
      const configData = transformApiConfigToComponentData(
  ...
  const fetchMembersData = async () => {
    ...
    try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          const stats = {
            ...
            regularMembers: membersList.filter(
              (m: any) => m.role === "ROLE_UNSPECIFIED"
            ).length,
      ...
      setMembersStats({
        ...
        regularMembers: fallbackMembers.filter(
          (m) => m.role === "ROLE_UNSPECIFIED"
        ).length,
  ...
  useEffect(() => {
    ...
    if (isOpen) {
      console.log(
        "✅ Modal abierto - iniciando carga de datos de organización"
      );
  ...
  const handleUnassignTag = async (tagId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/meet/spaces/${spaceId}/unassign-tags`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleAssignGroup = async (groupId: string) => {
    try {
      ...
      const spaceId = room?.name?.split("/").pop() || "";
      const response = await fetch(
        `/api/meet/spaces/${spaceId}/assign-groups`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleUnassignGroup = async (groupId: string) => {
    try {
      ...
      const spaceId = room?.name?.split("/").pop() || "";
      const response = await fetch(
        `/api/meet/spaces/${spaceId}/unassign-groups`,
        {
          method: "POST",
          credentials: "include",
          headers: { "Content-Type": "application/json" },
  ...
  const handleRoleChange = async (member: any, newRole: string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData.error || "Error al cambiar rol del miembro"
        );
      }
      ...
      setMembers((prevMembers) =>
        prevMembers.map((m) =>
          m.email === member.email
            ? { ...m, role: newRole, isCohost: newRole === "COHOST" }
            : m
        )
      );
      ...
      setMembersStats({
        ...
        regularMembers: updatedMembers.filter(
          (m) => m.role === "ROLE_UNSPECIFIED"
        ).length,
  ...
  const handleConfigChange = async (key: string, value: boolean | string) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(
          responseData.error || "Error al actualizar configuración"
        );
      }
  ...
  // Función para obtener fechas actualizadas de la API
  const fetchUpdatedDates = async () => {
    if (!room?.name) return;

    try {
      const roomId = room.name.split("/").pop();
      if (!roomId) return;

      console.log(
        `🔄 [FRONTEND] Obteniendo fechas actualizadas para ${roomId}`
      );
      const response = await fetch(`/api/meet/rooms/${roomId}/dates`, {
        method: "GET",
        headers: {
      ...
      if (response.ok) {
        ...
        console.log(`✅ [FRONTEND] Fechas actualizadas obtenidas:`, datesData);

        // Actualizar el objeto room localmente con las nuevas fechas
        if (room) {
          room.startDate = datesData.startDate;
          room.endDate = datesData.endDate;
          console.log(
            `🔄 [FRONTEND] Objeto room actualizado con nuevas fechas`
          );
        }
      }
    } catch (error) {
      console.error(
        `❌ [FRONTEND] Error obteniendo fechas actualizadas:`,
        error
      );
    }
  };
  ...
  const handleStartDateChange = async (date: Date | undefined) => {
    console.log(`🔄 [FRONTEND] handleStartDateChange iniciado:`, {
    ...
    if (!room?.name) {
      console.log(`❌ [FRONTEND] Error - room.name faltante:`, { room });
      return;
    }

    try {
      console.log(`🔄 [FRONTEND] Estableciendo loading state para startDate`);
      setIsUpdatingStartDate(true);

      // Extraer roomId del name (formato: "spaces/XXXX")
      const roomId = room.name.split("/").pop();
      console.log(`🎯 [FRONTEND] RoomId extraído de '${room.name}': ${roomId}`);

      if (!roomId) {
        const error = "ID de sala no válido";
        console.log(`❌ [FRONTEND] Error extrayendo roomId:`, {
          roomName: room.name,
          error,
        });
      ...
      const endpoint = `/api/meet/rooms/${roomId}/dates`;
      const payload = { startDate: date?.toISOString() };

      console.log(`📤 [FRONTEND] Preparando request:`, {
        method: "PATCH",
        endpoint,
        payload,
        headers: { "Content-Type": "application/json" },
      ...
      console.log(`🚀 [FRONTEND] Enviando request PATCH a ${endpoint}`);
      ...
      if (!response.ok) {
        ...
        console.error(`❌ [FRONTEND] Error en respuesta startDate:`, {
          status: response.status,
          statusText: response.statusText,
          errorData,
          roomId,
        ...
        throw new Error(
          `Error ${response.status}: ${errorData.error || errorText}`
        );
      }

      const result = await response.json();
      console.log(`✅ [FRONTEND] Éxito actualizando startDate:`, {
        result,
        roomId,
        originalDate: date?.toISOString(),
        newDate: result.room?.startDate,
      });

      const toastMessage = `La fecha de inicio se ${date ? "estableció" : "eliminó"} correctamente`;
      console.log(`🎉 [FRONTEND] Mostrando toast de éxito: ${toastMessage}`);
      ...
      // Obtener fechas actualizadas directamente
      console.log(`🔄 [FRONTEND] Obteniendo fechas actualizadas`);
      await fetchUpdatedDates();

      // Forzar recarga completa de datos
      console.log(`🔄 [FRONTEND] Forzando recarga completa de datos`);
      await queryClient.refetchQueries({ queryKey: ["rooms-list"] });

      // Actualizar datos
      console.log(`🔄 [FRONTEND] Llamando onUpdate para refrescar datos`);
      onUpdate?.();

      // Invalidar cache de React Query para forzar refresco de datos
      console.log(`🔄 [FRONTEND] Invalidando cache de React Query`);
      await queryClient.invalidateQueries({ queryKey: ["rooms-list"] });
      console.log(`✅ [FRONTEND] Cache invalidado exitosamente`);
    } catch (error) {
      const errorMessage =
      ...
      console.error(`💥 [FRONTEND] Error crítico en handleStartDateChange:`, {
        error: errorMessage,
      ...
      console.log(`🚨 [FRONTEND] Mostrando toast de error: ${errorMessage}`);
      toast({
        title: "Error",
      ...
      console.log(`🔄 [FRONTEND] Limpiando loading state para startDate`);
      setIsUpdatingStartDate(false);
    }
  };

  const handleEndDateChange = async (date: Date | undefined) => {
    console.log(`🔄 [FRONTEND] handleEndDateChange iniciado:`, {
      date: date?.toISOString(),
      roomName: room?.name,
      timestamp: new Date().toISOString(),
    });
    ...
    try {
      console.log(`🔄 [FRONTEND] Estableciendo loading state para endDate`);
      ...
      const endpoint = `/api/meet/rooms/${roomId}/dates`;
      const payload = { endDate: date?.toISOString() };
      ...
      toast({
        title: "Fecha actualizada",
        description: `La fecha de fin se ${date ? "estableció" : "eliminó"} correctamente`,
      });

      // Obtener fechas actualizadas directamente
      console.log(`🔄 [FRONTEND] Obteniendo fechas actualizadas para endDate`);
      await fetchUpdatedDates();

      // Forzar recarga completa de datos
      console.log(
        `🔄 [FRONTEND] Forzando recarga completa de datos para endDate`
      );
      await queryClient.refetchQueries({ queryKey: ["rooms-list"] });
      ...
      // Invalidar cache de React Query para forzar refresco de datos
      console.log(
        `🔄 [FRONTEND] Invalidando cache de React Query para endDate`
      );
      await queryClient.invalidateQueries({ queryKey: ["rooms-list"] });
      console.log(`✅ [FRONTEND] Cache invalidado exitosamente para endDate`);
    } catch (error) {
      toast({
        title: "Error",
        description:
...
function transformApiConfigToComponentData(apiSettings: any) {
  ...
  const entryPointAccess = EntryPointAccessEnum.safeParse(
    apiSettings.entryPointAccess
  );
  const chatRestriction = RestrictionTypeEnum.safeParse(
    apiSettings.moderationSettings?.chatRestriction
  );
  const reactionRestriction = RestrictionTypeEnum.safeParse(
    apiSettings.moderationSettings?.reactionRestriction
  );
  const presentRestriction = RestrictionTypeEnum.safeParse(
    apiSettings.moderationSettings?.presentRestriction
  );
  const autoRecording = AutoGenerationTypeEnum.safeParse(
    apiSettings.recordingSettings?.autoRecordingGeneration
  );
  const autoTranscription = AutoGenerationTypeEnum.safeParse(
    apiSettings.transcriptionSettings?.autoTranscriptionGeneration
  );
  const smartNotes = AutoGenerationTypeEnum.safeParse(
    apiSettings.smartNotesSettings?.autoSmartNotesGeneration
  );
  return {
    moderation: {
      restrictEntry: {
        enabled: entryPointAccess.success
          ? entryPointAccess.data === "CREATOR_APP_ONLY"
          : false,
        label: "Restringir Puntos de Entrada",
        description:
          "Solo los puntos de entrada del proyecto Google Cloud que creó el espacio pueden unirse",
        tooltip:
          "entryPointAccess: ALL permite todos los puntos de entrada, CREATOR_APP_ONLY solo puntos de entrada propios",
      },
      enableModeration: {
        enabled: apiSettings.moderationSettings?.moderationEnabled || false,
        label: "Activar Moderación",
        description:
          "Da al propietario de la reunión más control sobre funciones y participantes",
        tooltip:
          "moderation: Cuando está ON permite gestionar co-anfitriones y restricciones de funciones",
      },
      chatRestriction: {
        value: chatRestriction.success
          ? chatRestriction.data
          : "NO_RESTRICTION",
        options: [
          {
            value: "NO_RESTRICTION",
            label: "Todos los participantes tienen permisos",
          },
          {
            value: "HOSTS_ONLY",
            label: "Solo el organizador y co-anfitriones tienen permisos",
          },
        ],
      },
      reactionRestriction: {
        value: reactionRestriction.success
          ? reactionRestriction.data
          : "NO_RESTRICTION",
        options: [
          {
            value: "NO_RESTRICTION",
      ...
      presentationRestriction: {
        value: presentRestriction.success
          ? presentRestriction.data
          : "NO_RESTRICTION",
        options: [
          {
            value: "NO_RESTRICTION",
      ...
      defaultViewer: {
        ...
        description:
          "Los usuarios se unirán como espectadores (ON) o contribuyentes (OFF) por defecto",
        tooltip:
          "defaultJoinAsViewerType: Define si restringir el rol por defecto asignado a usuarios como espectador",
      },
    },
    aiFeatures: {
      autoRecording: {
        ...
        description:
          "Define si un espacio de reunión se graba automáticamente cuando se une alguien con privilegios de grabación",
        tooltip:
          "recordingConfig.autoRecordingGeneration: Graba automáticamente reuniones cuando se une usuario autorizado",
      },
      autoTranscription: {
        enabled: autoTranscription.success
          ? autoTranscription.data === "ON"
          : false,
        label: "Generación Automática de Transcripción",
        description:
          "Define si el contenido de la reunión se transcribe automáticamente cuando se une usuario autorizado",
        tooltip:
          "transcriptionConfig.autoTranscriptionGeneration: Transcribe automáticamente contenido de reunión cuando se une usuario autorizado",
      },
      smartNotes: {
        enabled: smartNotes.success ? smartNotes.data === "ON" : false,
        label: "Generación Automática de Notas Inteligentes",
        description:
          "Define si generar automáticamente resumen y recapitulación para todos los invitados cuando se une usuario autorizado",
        tooltip:
          "smartNotesConfig.autoSmartNotesGeneration: Genera automáticamente resumen y recapitulación de reunión para todos los invitados de la organización cuando se une usuario autorizado",
      },
    },
  };
}
...
function generateMockConfigData() {
  return {
    ...
    alert: {
      message:
        "Configuraciones de demostración. Los cambios reales requieren conexión a la API.",
    },
  };
}
...
function updateConfigurationLocally(
  config: any,
  key: string,
