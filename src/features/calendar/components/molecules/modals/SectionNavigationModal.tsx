/**
 * SECTIONNAVIGATIONMODAL - Modal responsivo con navegación para eventos de calendario
 * Integra el sistema de navegación con ResponsiveModal
 *
 * Características:
...
export const SectionNavigationModal: React.FC<SectionNavigationModalProps> = ({
  ...
  const navigation = useModalNavigation({
    ...
    enableHistory: true,
    persistState: true,
    storageKey: `calendar-modal-navigation-${title.toLowerCase().replace(/\s+/g, "-")}`,
    onSectionChange: (sectionId, previousSectionId) => {
      console.log(`🧭 Calendar Navigation: ${previousSectionId} → ${sectionId}`);
      if (onSectionChange) {
  ...
  console.log('🧭 Calendar SectionNavigationModal: Rendering section', {
    currentSectionId,
          ...
          <ScrollArea className='h-full px-1'>
            <div className={cn("pb-2", customFooter && "pb-0")}>
              {CurrentSectionComponent ? (
                <Suspense fallback={
                  (() => {
                    console.log('🧭 Calendar SectionNavigationModal: Showing loading fallback for section:', currentSectionId);
                    return <SectionLoadingFallback />;
                  })()
                }>
                  {(() => {
                    console.log('🧭 Calendar SectionNavigationModal: Attempting to render component for section:', currentSectionId);
                    try {
                      return <CurrentSectionComponent {...combinedProps} />;
                    } catch (error) {
                      console.error('🧭 Calendar SectionNavigationModal: Error rendering component for section:', currentSectionId, error);
                      return (
