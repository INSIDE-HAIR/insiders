/**
 * ParticipantKPISection - Organism Component
 * 
 * Section IDÉNTICO estéticamente al original ParticipantKPIGrid.tsx
 * + Estado de loading con skeleton
 * Copiado exacto de: ParticipantKPIGrid.tsx líneas completas 1-439
 * Usando molecules y atoms para composición
 */
...
interface ParticipantKPISectionProps {
...
export const ParticipantKPISection: React.FC<ParticipantKPISectionProps> = ({
  selectedAttendees,
  events,
  onRemoveAttendee,
  calendarIds,
  ...
  const isStoreLoading = useParticipantKPILoading();
  ...
  // Get KPIs for selected attendees - copiado exacto líneas 76-81
  ...
  // Calculate unique session-based KPIs - copiado exacto líneas 83-206
  ...
  // Handle remove attendee - copiado exacto líneas 208-212
  const handleRemoveAttendee = (email: string) => {
    removeKPI(email);
    onRemoveAttendee(email);
  };

  // Handle retry - copiado exacto líneas 214-221
  const handleRetry = () => {
    fetchKPIs(selectedAttendees, {
      startDate: dateRange?.start,
      endDate: dateRange?.end,
  ...
  // If error, show error state - copiado exacto líneas 238-260
  if (error && !isLoading) {
    return (
      <div className="mb-6">
  ...
  return (
    <div className="mb-6 space-y-4">
      {/* Header - copiado exacto líneas 264-284 */}
      ...
      {/* KPI Cards Grid - copiado exacto líneas 286-340 pero usando ParticipantCard molecule */}
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
      ...
      {/* Show more button if there are many attendees - copiado exacto líneas 342-352 */}
      ...
      {/* Summary stats - Based on unique sessions - copiado exacto líneas 354-436 */}
...
ParticipantKPISection.displayName = "ParticipantKPISection";