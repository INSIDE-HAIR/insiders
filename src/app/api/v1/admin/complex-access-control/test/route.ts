...
import {
  evaluateComplexAccess,
  EvaluationContext,
} from "@/src/lib/rule-engine/complex-rule-evaluator";
import { z } from "zod";
...
export async function POST(request: NextRequest) {
  try {
    ...
    const simulatedDate = validatedData.simulatedDate
      ? new Date(validatedData.simulatedDate)
      : new Date();
    const simulatedTime =
      validatedData.simulatedTime || simulatedDate.toTimeString().slice(0, 5);
    const simulatedDay = simulatedDate.toLocaleDateString("en-US", {
      weekday: "long",
    });
    ...
    const context: EvaluationContext = {
      user: validatedData.user
        ? {
            ...
            deactivation_date: validatedData.user.deactivation_date
              ? new Date(validatedData.user.deactivation_date)
              : undefined,
            subscription_end_date: validatedData.user.subscription_end_date
              ? new Date(validatedData.user.subscription_end_date)
              : undefined,
            last_login: validatedData.user.last_login
              ? new Date(validatedData.user.last_login)
              : undefined,
          }
        : null,
    ...
    const result = await evaluateComplexAccess(
  ...
  try {
    ...
    const testCases = [
      ...
      },
      {
        ...
        description:
          "Usuario inscrito en edición enero después de fecha límite",
        resourceId: "marketing_digital_avanzado",
        user: {
          id: "user1",
          email: "juan.perez@empresa.com",
      ...
      },
      {
        ...
        description:
          "Usuario con servicio premium dado de baja hace 6 meses (dentro del año de gracia)",
        resourceId: "marketing_digital_avanzado",
        user: {
          id: "user3",
