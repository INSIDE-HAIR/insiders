    ...
    for (const eventRequest of events) {
      try {
        ...
        const eventForm = {
          ...
          endTime: event.end?.dateTime ? event.end.dateTime.split('T')[1].substring(0, 5) : undefined,
        ...
        const newEvent = await calendarService.createEvent(eventForm, targetCalendarId);
        ...
        const deleted = await calendarService.deleteEvent(sourceCalendarId, eventId);
        ...
        if (!deleted) {
          ...
          console.warn(`Failed to delete original event ${eventId} from ${sourceCalendarId} after copying to ${targetCalendarId}`);
        }
