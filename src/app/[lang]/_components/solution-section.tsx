        ...
        <div className="grid md:grid-cols-2 gap-12 lg:gap-16 items-center">
          <div className="relative">
            ...
            <p className="text-lg text-muted-foreground mb-8">
              No es un curso más, es un sistema de implementación. Te entregamos
              los procesos, las herramientas y el acompañamiento para tomar el
              control de las 5 áreas clave de tu negocio, transformando tu salón
              en una empresa rentable y autónoma.
            </p>
            <div className="space-y-3">
              {areas.map((area) => (
                <p
                  key={area}
                  className="flex items-center text-lg text-foreground/90"
                >
                  <Icons.CheckSquare className="text-primary mr-3 h-5 w-5 flex-shrink-0" />{" "}
                  {area}
                </p>
              ))}
            </div>
          ...
          <div className="relative flex items-center justify-center h-80 w-80 md:h-96 md:w-96 mx-auto">
            ...
            {areas.map((label, i) => {
              ...
              const x =
                50 + radiusPercentage * Math.cos((angle * Math.PI) / 180);
              const y =
                50 + radiusPercentage * Math.sin((angle * Math.PI) / 180);
              return (
                <div
                  key={label}
