...
export default function AddingComponentsPage() {
  return (
    <div>
      <DocHeader title="Añadir Componentes" description="Guía paso a paso para añadir nuevos tipos de componentes" />

      <DocContent>
        <h2 className="text-2xl font-bold mt-8 mb-4">Cómo añadir un nuevo tipo de componente</h2>

        <p className="mb-4">
          Esta guía te mostrará cómo añadir un nuevo tipo de componente a la plataforma. Seguiremos un proceso paso a
          paso para integrar completamente el componente en el sistema.
        </p>

        <h3 className="text-xl font-semibold mt-6 mb-3">Paso 1: Crear el componente renderizador</h3>

        <p className="mb-2">
          Crea un nuevo archivo en <code>components/renderers/</code> para tu componente:
        </p>
  ...
  return (
    <div className="w-full max-w-4xl mx-auto mb-8">
      ...
      <div>Mi nuevo componente: {item.displayName}</div>
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">Paso 2: Actualizar el ComponentSelector</h3>

        <p className="mb-2">
          Actualiza el <code>ComponentSelector</code> para incluir el nuevo tipo:
        </p>
...
import { MiComponenteRenderer } from "@/components/renderers/mi-componente-renderer"
...
switch (componentType) {
  ...
  default:
    return <GenericRenderer item={item} contentType={getContentType(item)} />
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">Paso 3: Actualizar RecursiveContentRenderer</h3>

        <p className="mb-2">
          Actualiza el <code>RecursiveContentRenderer</code> para manejar el nuevo tipo:
        </p>
  ...
  <div className="w-full flex flex-col items-center my-6 max-w-4xl mx-auto">
    {miComponenteItems.map((item) => (
      <div key={item.id} className="w-full max-w-4xl mb-8">
        ...
        <h3 className="text-xl font-semibold mt-6 mb-3">Paso 4: Añadir una función de utilidad (opcional)</h3>

        <p className="mb-2">
          Si tu componente utiliza propiedades personalizadas del campo description, añade una función de utilidad:
        </p>
...
* 
* @param {ContentItem} item - El elemento de contenido
* @returns {string|null} El valor de mi propiedad o null si no existe
*/
export function extractMiPropiedad(item: ContentItem): string | null {
  return extractPropertyFromDescription(item, "miPropiedad");
}`}
        </pre>

        <h3 className="text-xl font-semibold mt-6 mb-3">Paso 5: Documentar el nuevo componente</h3>

        <p className="mb-4">
          Es importante documentar el nuevo componente para que los usuarios sepan cómo utilizarlo:
        </p>

        <ol className="list-decimal pl-5 space-y-2 mb-6">
          <li>Actualiza la documentación de usuarios para incluir el nuevo tipo de componente</li>
          <li>Documenta el formato esperado para el campo description (si aplica)</li>
          <li>Proporciona ejemplos de uso</li>
        </ol>

        <h3 className="text-xl font-semibold mt-6 mb-3">Ejemplo completo</h3>

        <p className="mb-4">Veamos un ejemplo completo de cómo añadir un componente para mostrar un contador:</p>

        <h4 className="text-lg font-medium mt-4 mb-2">1. Crear el componente renderizador</h4>
  ...
  return (
    <div className="w-full max-w-md mx-auto p-4 bg-zinc-100 rounded-md">
      ...
      <div className="flex items-center justify-center gap-4">
        ...
        <h4 className="text-lg font-medium mt-4 mb-2">2. Añadir la función de utilidad</h4>
...
*/
export function extractInitialCount(item: ContentItem): string | null {
  return extractPropertyFromDescription(item, "initialCount");
}`}
        </pre>

        <h4 className="text-lg font-medium mt-4 mb-2">3. Actualizar el determineComponentType</h4>
...
export function determineComponentType(item: ContentItem): string {
  ...
  return "generic"
        ...
        <h4 className="text-lg font-medium mt-4 mb-2">4. Actualizar el ComponentSelector</h4>

        <pre className="bg-zinc-900 text-zinc-100 p-3 rounded text-sm overflow-x-auto mb-4">
          {`// En components/selectors/component-selector.tsx

// Importar el nuevo componente
import { CounterRenderer } from "@/components/renderers/counter-renderer"
    ...
    return <GenericRenderer item={item} contentType={getContentType(item)} />
        ...
        <div className="bg-blue-50 border-l-4 border-blue-500 p-4 my-6">
          <p className="text-blue-800">
            <strong>Consejo:</strong> Sigue este patrón para añadir cualquier tipo de componente nuevo. La clave está en
            mantener la coherencia con el sistema existente y documentar adecuadamente el nuevo componente.
          </p>
        </div>
      </DocContent>
    </div>
