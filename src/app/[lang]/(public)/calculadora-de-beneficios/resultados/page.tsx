...
export default function ResultadosPage() {
  ...
  useEffect(() => {
    ...
    } else if (currentStep !== CURRENT_PAGE_STEP && !expectedPath && pathname === stepToPathMap[CURRENT_PAGE_STEP]) {
  ...
  const getProfitColor = () => {
    if (profit_percentage === null || profit_percentage === undefined || !Number.isFinite(profit_percentage))
      return "text-brand-white"
    return profit_percentage < 10 ? "text-red-500" : "text-green-500"
  }

  const formatCurrency = (value: number | undefined | null) => {
    if (value === undefined || value === null || !Number.isFinite(value)) return "Calculando..."
  ...
  if (total_expenses_no_vat === undefined || total_team_hours === undefined || total_income_no_vat === undefined) {
    return (
      <div className="w-full max-w-xl p-6 sm:p-8 space-y-6 bg-black rounded-lg shadow-xl text-brand-white text-center">
        ...
        <p className="text-gray-300">
          Parece que faltan algunos datos para calcular tus resultados. Por favor, comienza la calculadora desde el
          principio.
        </p>
        <Button onClick={handleReset} className="mt-4 bg-brand-cta-accent text-black hover:bg-opacity-80">
  ...
  return (
    <div className="w-full max-w-xl p-6 sm:p-8 space-y-6 bg-black rounded-lg shadow-xl text-brand-white">
      <h2 className="text-2xl sm:text-3xl font-bold text-center">
        {full_name ? `${full_name}, estos son tus resultados:` : "Tus Resultados de Rentabilidad"}
      </h2>

      <p className="text-center text-gray-300 text-sm sm:text-base">
        Estos son tus resultados de rentabilidad. Aquí puedes ver si tu peluquería está generando los márgenes
        esperados.
      </p>
      ...
      {leadSaved && !error && <p className="text-green-500 text-center py-2">¡Tus resultados han sido guardados!</p>}

      <div className="space-y-4 pt-4">
        <div className="p-4 bg-brand-gray-dark rounded-md">
          <h3 className="text-lg font-semibold text-gray-300">Tus Gastos/Hora:</h3>
          <p className="text-2xl font-bold text-brand-cta-accent">{formatCurrency(expenses_per_hour)}</p>
        </div>
        <div className="p-4 bg-brand-gray-dark rounded-md">
          <h3 className="text-lg font-semibold text-gray-300">Tus Ingresos/Hora:</h3>
          <p className="text-2xl font-bold text-brand-cta-accent">{formatCurrency(income_per_hour)}</p>
        </div>
        <div className="p-4 bg-brand-gray-dark rounded-md">
          <h3 className="text-lg font-semibold text-gray-300">Tus Beneficios:</h3>
          <p className={`text-2xl font-bold ${getProfitColor()}`}>{formatPercentage(profit_percentage)}</p>
        </div>
      </div>

      <p className="text-sm text-center text-gray-400 pt-4">
        Estos cálculos son estimaciones. Para un análisis financiero detallado, y profesional, puedes acceder a nuestra
      ...
      <div className="flex flex-col sm:flex-row justify-center items-center gap-4 pt-6">
        ...
        <Button asChild className="w-full sm:w-auto bg-brand-cta-accent text-black hover:bg-opacity-80">
          <Link
