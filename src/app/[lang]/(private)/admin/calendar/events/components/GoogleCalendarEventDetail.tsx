...
interface GoogleCalendarEventDetailProps {
...
export const GoogleCalendarEventDetail: React.FC<
  GoogleCalendarEventDetailProps
> = ({ event, isOpen, onClose, onSave, onDelete, calendars }) => {
  const [editedEvent, setEditedEvent] =
    useState<Partial<GoogleCalendarEvent>>(event);
  const [isEditing, setIsEditing] = useState(false);
  const [activeTab, setActiveTab] = useState<
    "details" | "find-time" | "guests"
  >("details");
  ...
  const handleAddAttendees = async (newAttendees: string[]) => {
    try {
      const attendeesToAdd = newAttendees.map((email) => ({ email }));
      ...
      const response = await fetch(
        ...
        }`,
        {
          ...
          body: JSON.stringify({ attendees: attendeesToAdd, action: "add" }),
        }
      );

      if (!response.ok) throw new Error("Error adding attendees");

      const result = await response.json();
      setEditedEvent({ ...editedEvent, attendees: result.attendees });
    } catch (error) {
