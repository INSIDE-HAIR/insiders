...
export default function GroupsClient() {
  ...
  const handleAddUsersToGroup = async () => {
    ...
    try {
      const response = await fetch(`/api/admin/groups/${selectedGroup.id}/users`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
          ...
          {filteredGroups.length === 0 ? (
            <div className="text-center py-8">
              ...
              <p className="text-muted-foreground mb-4">
                {searchTerm ? "No se encontraron grupos con ese criterio" : "Crea tu primer grupo para organizar usuarios"}
              </p>
              {!searchTerm && (
                    ...
                    <TableCell className="text-right">
                      <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                          <DropdownMenuItem onClick={() => openManageUsersDialog(group)}>
      ...
      <Dialog open={isManageUsersDialogOpen && !!selectedGroup} onOpenChange={(open) => {
        ...
        <DialogContent className="max-w-4xl max-h-[80vh]">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 overflow-hidden">
            {/* Current Users */}
            <div className="space-y-4">
              <h3 className="font-semibold">Usuarios en el Grupo ({selectedGroup?.users.length || 0})</h3>
            ...
            <div className="space-y-4">
              ...
              {selectedUsers.length > 0 && (
                <Button onClick={handleAddUsersToGroup} className="w-full">
                  <UserPlus className="h-4 w-4 mr-2" />
                  Añadir {selectedUsers.length} Usuario{selectedUsers.length !== 1 ? 's' : ''}
                </Button>
              )}
            </div>
          </div>
