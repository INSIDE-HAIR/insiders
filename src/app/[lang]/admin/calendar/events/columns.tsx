...
export const useEventsColumns = (
  ...
  const getEventStatus = (event: GoogleCalendarEvent) => {
    ...
    if (now < start) return { status: 'upcoming', color: 'blue', label: 'Próximo' };
    ...
    switch (columnId) {
      ...
      case 'end':
        return {
          ...
          cell: ({ row }) => {
            ...
            if (isAllDay) {
              return <div className="text-sm">{date.toLocaleDateString('es-ES', { month: 'short', day: 'numeric' })}</div>;
            }
            return <div className="text-sm">{date.toLocaleDateString('es-ES', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' })}</div>;
          },
        };
      ...
      case 'status':
        return {
          ...
          cell: ({ row }) => {
            ...
            return (
              <Badge 
                variant={eventStatus.color === 'blue' || eventStatus.color === 'green' ? 'default' : 'secondary'}
                className={
                  eventStatus.color === 'green' ? 'bg-green-100 text-green-800 hover:bg-green-200' :
                  eventStatus.color === 'blue' ? 'bg-blue-100 text-blue-800 hover:bg-blue-200' :
      ...
      case 'location':
        return {
          ...
          header: "Ubicación",
          cell: ({ row }) => (
            <div className="text-sm text-gray-600 truncate max-w-[200px]" title={event(row).location}>
              ...
              <div className="text-sm max-w-[400px]">
                ...
                <div className="flex gap-3 mb-2 p-2 bg-blue-50 rounded">
                  {statusCounts.accepted > 0 && (
                    <div className="flex items-center gap-1 text-xs">
                      ...
                      <span className="font-medium text-green-700">{statusCounts.accepted} ✓</span>
                    </div>
                  )}
                  {statusCounts.declined > 0 && (
                    <div className="flex items-center gap-1 text-xs">
                      <span className="w-2 h-2 bg-red-500 rounded-full"></span>
                      <span className="font-medium text-red-700">{statusCounts.declined} ✗</span>
                    </div>
                  )}
                  {statusCounts.tentative > 0 && (
                    <div className="flex items-center gap-1 text-xs">
                      <span className="w-2 h-2 bg-yellow-500 rounded-full"></span>
                      <span className="font-medium text-yellow-700">{statusCounts.tentative} ?</span>
                    </div>
                  )}
                  {statusCounts.needsAction > 0 && (
                    <div className="flex items-center gap-1 text-xs">
                      <span className="w-2 h-2 bg-gray-400 rounded-full"></span>
                      <span className="font-medium text-gray-600">{statusCounts.needsAction} ⏳</span>
                    </div>
                  )}
                </div>
                ...
                <div className="space-y-1 max-h-32 overflow-y-auto">
                  {attendees.map((attendee, index) => {
                    ...
                    const config = statusConfig[attendee.responseStatus as keyof typeof statusConfig] || statusConfig.needsAction;
                ...
                <div className="text-xs text-gray-500 mt-2 font-medium border-t pt-1">
                  Total: {attendees.length} invitado{attendees.length !== 1 ? 's' : ''}
                </div>
              </div>
            );
          },
          filterFn: (row, id, value: string[]) => {
            ...
            return value.some(filterEmail => 
              attendees.some(attendee => 
                attendee.email?.toLowerCase().includes(filterEmail.toLowerCase()) ||
                attendee.displayName?.toLowerCase().includes(filterEmail.toLowerCase())
              )
            );
          },
        };
      ...
      case 'description':
        return {
          ...
          header: "Descripción",
          cell: ({ row }) => (
            <div className="text-sm text-gray-500 truncate max-w-[300px]" title={event(row).description}>
      ...
      case 'creator':
        return {
          ...
          cell: ({ row }) => {
            ...
            const creatorCalendar = calendars.find(cal => cal.id === creator.email);
              ...
              <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex-1">
                  <div className="font-medium text-gray-900">
                    {creator.displayName || creator.email?.split('@')[0] || 'Sin nombre'}
                  </div>
      ...
      case 'organizer':
        return {
          ...
          cell: ({ row }) => {
            ...
            const organizerCalendar = calendars.find(cal => cal.id === organizer.email);
            const calendarColor = organizerCalendar ? {
              backgroundColor: organizerCalendar.backgroundColor || '#4285f4',
              foregroundColor: organizerCalendar.foregroundColor || '#ffffff',
            ...
            const copyShareLink = () => {
              ...
              if (calendarId) {
                const shareUrl = `https://calendar.google.com/calendar/u/1?cid=${encodeURIComponent(btoa(calendarId))}`;
                navigator.clipboard.writeText(shareUrl);
                toast({
                  title: "Enlace de calendario copiado",
                  description: "Enlace para compartir calendario copiado al portapapeles",
                  duration: 3000,
                });
              }
            };
            ...
            const copyIframeEmbed = () => {
              ...
              if (calendarId) {
                ...
                toast({
                  ...
                  description: "Código para embeber calendario copiado al portapapeles",
                  duration: 3000,
                });
              }
            };
            ...
            return (
              <div className="flex items-center gap-2 text-sm">
                ...
                <div className="flex-1">
                  <div className="font-medium text-gray-900">
                    {organizer.displayName || organizer.email?.split('@')[0] || 'Sin nombre'}
                  </div>
                  {calendarColor && (
                    <div className="text-xs text-blue-600">
                      📅 Calendario
                ...
                <div className="flex items-center gap-1">
                  ...
                  <Button
                    ...
                    title={`Copiar ID del organizador: ${organizer.email || 'N/A'}`}
                  >
      ...
      case 'visibility':
        return {
          ...
          cell: ({ row }) => {
            ...
            return (
              <Badge className={colors[visibility as keyof typeof colors] || colors.default}>
                {visibility}
              </Badge>
            );
          },
      ...
      case 'transparency':
        return {
          ...
          cell: ({ row }) => {
            ...
            return (
              <Badge variant={transparency === 'transparent' ? 'outline' : 'default'}>
          ...
          cell: ({ row }) => {
            ...
            return (
              <div className="space-y-1">
                ...
                {conferenceData?.entryPoints?.some((ep: any) => ep.entryPointType === 'video') && (
              ...
              <div className="text-sm max-w-[300px] space-y-2">
                  ...
                  <div className="flex items-center gap-2 p-2 bg-blue-50 rounded">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="text-xs text-blue-700">
                        {conferenceData.conferenceSolution.iconUri ? 'Con icono' : 'Estándar'}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {conferenceData?.conferenceId && (
                  <div className="p-2 bg-gray-50 rounded">
                    <div className="text-xs text-gray-500 mb-1">ID de Conferencia:</div>
                    ...
                    {conferenceData.entryPoints.map((entry: any, index: number) => (
                      <div key={index} className="flex items-center gap-2 p-1 bg-gray-50 rounded text-xs">
                        ...
                        <div className="flex-1 min-w-0">
                          {entry.entryPointType === 'video' && (
                            <div>
                              <div className="font-medium text-green-700">Video</div>
                              {entry.uri && (
                                <button 
                          ...
                          {entry.entryPointType === 'phone' && (
                            <div>
                              <div className="font-medium text-blue-700">Teléfono</div>
                              <div className="font-mono text-gray-600">
                                {entry.uri?.replace('tel:', '') || 'No disponible'}
                              </div>
                              {entry.pin && (
                                <div className="text-gray-500">PIN: {entry.pin}</div>
                              )}
                            </div>
                          )}
                          {entry.entryPointType === 'sip' && (
                            <div>
                              <div className="font-medium text-gray-700">SIP</div>
                          ...
                          {entry.entryPointType === 'more' && (
                            <div>
                              <div className="font-medium text-purple-700">Más opciones</div>
                              ...
                              {entry.pin && (
                                <div className="text-xs text-purple-700 bg-purple-50 px-1 rounded mt-1">
                                  PIN: <span className="font-mono font-bold">{entry.pin}</span>
                                </div>
                              )}
                            </div>
                          )}
                ...
                {!conferenceData && hangoutLink && (
                  <div className="p-2 bg-green-50 rounded">
                    <div className="text-xs text-green-700 mb-1">Google Meet (Legacy)</div>
                    <button 
                      onClick={() => window.open(hangoutLink, '_blank')}
      ...
      case 'meetingPhone':
        return {
          ...
          cell: ({ row }) => {
            const conferenceData = event(row).conferenceData;
            const phoneEntries = conferenceData?.entryPoints?.filter((ep: any) => ep.entryPointType === 'phone') || [];
            
            if (phoneEntries.length === 0) {
              return <span className="text-gray-500">Sin acceso telefónico</span>;
            }
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <div className="flex-1">
                        <div className="font-mono text-sm text-blue-900">
                          {phone.label || phone.uri?.replace('tel:', '') || 'No disponible'}
                        </div>
                        {phone.pin && (
      ...
      case 'recurrence':
        return {
          ...
          cell: ({ row }) => {
            ...
            if (!recurrence || recurrence.length === 0) return <span className="text-gray-500">-</span>;
            return (
          ...
          cell: ({ row }) => {
            ...
            return (
              <div className="flex items-center gap-2 text-sm">
                ...
                />
                <div className="flex-1 min-w-0">
                  <div className="font-medium text-gray-900 truncate" title={calendarColor.summary}>
                    {calendarColor.summary}
                  </div>
                  <div className="text-xs text-gray-500 font-mono truncate" title={calendarId}>
                    {calendarId.length > 20 ? `${calendarId.substring(0, 20)}...` : calendarId}
                  </div>
                </div>
              </div>
            );
          ...
          cell: ({ row }) => (
            <div className="flex items-center gap-1">
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => router.push(`/admin/calendar/events/${event(row).id}?calendarId=${(event(row) as any).calendarId || defaultCalendarId}`)}
                className="h-8 w-8 p-0"
                title="Editar evento"
              >
              ...
              <Button
                variant="ghost"
                size="sm"
                onClick={() => handleDeleteEvent(event(row).id!, (event(row) as any).calendarId || defaultCalendarId)}
      ...
      default:
        ...
        return {
          ...
          header: AVAILABLE_COLUMNS[columnId as keyof typeof AVAILABLE_COLUMNS]?.label || columnId,
          cell: ({ row }) => {
            ...
            return (
              <div className="text-sm truncate max-w-[200px]" title={String(value)}>
                {String(value)}
              </div>
            );
          },
  ...
  const columns = visibleColumns.map(columnId => generateColumn(columnId)).filter(Boolean);
