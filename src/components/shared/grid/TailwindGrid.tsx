...
function TailwindGrid({
  ...
  const currentPadding =
    fullSize || padding === ""
      ? ""
      : rtlSupport
      ? CONFIG.paddingsRTL[padding]
      : CONFIG.paddings[padding];

  // Generar clases de ancho máximo responsivas
  const maxWidthClasses = generateMaxWidthClasses(
    maxWidth,
    mdMaxWidth,
    lgMaxWidth
  );
  ...
  return (
    <>
        ...
        {show && (
          <div
            className={cn(
              ...baseGridClasses,
              currentPadding,
              "absolute w-full z-50 box-border"
            )}
          >
      ...
      <section className={contentSectionClasses}>{children}</section>
    </>
  );
}
