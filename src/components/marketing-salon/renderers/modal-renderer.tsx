 ...
 *
 * @param {ContentItem} item - Elemento de tipo carpeta con prefijo "modal"
 * @returns Botón que abre un modal con los contenidos de la carpeta
 */
export function ModalRenderer({ item }: { item: ContentItem }) {
            ...
            {item.children && item.children.length > 0 && (
              <div className='w-full h-full flex items-center justify-center'>
                {item.children.map((child, index) => (
                  ...
                  >
                    {child.transformedUrl?.preview ? (
                      <img
                        src={child.transformedUrl.preview || "/placeholder.svg"}
