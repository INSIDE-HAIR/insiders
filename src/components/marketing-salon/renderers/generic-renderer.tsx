...
import { decodeFileName } from "@/src/lib/utils/marketing-salon/file-decoder";
import { extractCopyText } from "@/src/lib/utils/marketing-salon/description-parser";
import { useSearchParams } from "next/navigation";
// Actualizar la importación de ImageBlurUp
import { ImageBlurUp } from "@/src/components/marketing-salon/ui/image-components/image-blur-up";
...
// Definir una interfaz para las propiedades CSS personalizadas
interface CustomScrollbarStyle extends React.CSSProperties {
 ...
 *
 * @param {ContentItem} item - Elemento de contenido a renderizar
...
export function GenericRenderer({ item, contentType }: GenericRendererProps) {
  ...
  const currentModalPreview =
    item.previewItems && item.previewItems.length > 0
      ? item.previewItems[currentModalPreviewIndex]
      : null;
  ...
  const nextCardPreview = () => {
    if (item.previewItems && item.previewItems.length > 0) {
      ...
      setCurrentCardPreviewIndex(
        (prev) => (prev + 1) % item.previewItems!.length
      );
    }
  };
  ...
  const hasPreview =
    currentCardPreview?.transformedUrl?.preview ||
    (item.transformedUrl && item.transformedUrl.preview);
  const hasEmbed =
    currentModalPreview?.transformedUrl?.embed ||
    (item.transformedUrl && item.transformedUrl.embed);
            ...
            {hasMultiplePreviews && (
              <>
                ...
                <div className='absolute bottom-1 left-0 right-0 flex justify-center gap-1'>
                  {item.previewItems?.map((_, index: number) => (
                    <div
                      key={index}
      ...
      <div className='p-2 mt-2 flex gap-2 flex-wrap'>
        {item.transformedUrl?.download && (
          <div className='flex w-full'>
            ...
            >
              <a
                href={item.transformedUrl.download}
                target='_blank'
          ...
          style={
            {
              ...
              // Add webkit scrollbar styles directly to this element
              "--scrollbar-width": "8px",
              "--scrollbar-height": "8px",
              "--scrollbar-track-color": "#3f3f46",
              "--scrollbar-thumb-color": "#CEFF66",
            } as CustomScrollbarStyle
          }
        >
              ...
              {contentType === "Presentación" ? (
                ...
                <div className='w-full h-full flex items-center justify-center'>
                  <div className='relative w-full max-w-[600px] h-[400px] flex items-center justify-center'>
                    ...
                    {hasMultiplePreviews && !isModalImageLoading && (
                      <>
                        <button
