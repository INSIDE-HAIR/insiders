 ...
 *
 * @param {ContentItem} item - Elemento de contenido que contiene la información de la presentación
...
export function GoogleSlidesRenderer({ item }: GoogleSlidesRendererProps) {
  ...
  useEffect(() => {
    ...
    if (item.transformedUrl?.embed || item.transformedUrl?.preview) {
      ...
      const embedUrl = item.transformedUrl.embed;
      ...
      if (embedUrl && embedUrl.includes("docs.google.com/presentation")) {
