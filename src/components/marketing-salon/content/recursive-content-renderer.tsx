...
import { TabNavigation } from "@/src/components/marketing-salon/navigation/tab-navigation";
import { hasContent } from "@/src/components/marketing-salon/navigation/tab-navigation";
import { SectionNavigation } from "@/src/components/marketing-salon/navigation/section-navigation";
import { ContentGrid } from "@/src/components/marketing-salon/content/content-grid";
import { ComponentSelector } from "@/src/components/marketing-salon/selectors/component-selector";
...
export function RecursiveContentRenderer({
  ...
  const getAllItems = (): ContentItem[] => {
    if (parentId) {
      ...
      return (
        parent?.children
          .filter((item: ContentItem) => hasContent(item))
  ...
  const tabItems = tabsFolder
    ? tabsFolder.children.filter((item: ContentItem) =>
        item.prefixes.includes("tab")
      )
    : allItems.filter((item) => item.prefixes.includes("tab"));
  ...
  const directItems = allItems.filter(
    (item) =>
      ...
      !item.prefixes.includes("tab") && // No es un tab
      !item.prefixes.includes("section") // No es una sección
  );
  ...
  const buttonItems = directItems.filter(
    (item) =>
      item.prefixes.includes("button") && !googleFormItems.includes(item)
  );
  ...
  const googleSlidesItems = directItems.filter(
    (item) =>
      ...
      item.prefixes.includes("googleslide") ||
  ...
  const sectionItems = allItems.filter((item) =>
    item.prefixes.includes("section")
  );
  ...
  return (
    <div className='w-full flex flex-col items-center max-w-screen-2xl mx-auto'>
      ...
      {currentPathItem &&
        tabItems.some(
          (item: ContentItem) => item.id === currentPathItem.id
        ) && (
