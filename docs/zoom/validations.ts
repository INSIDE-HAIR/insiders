...
export const zoomDateRangeSchema = z
  .object({
    from: z
      .string()
...
export const zoomOAuthConfigSchema = z.object({
  ...
  scope: z
    ...
    .optional()
    .default(
      "meeting:write meeting:read webinar:write webinar:read recording:read user:read"
    ),
...
export const zoomTokenRequestSchema = z
  .object({
    grant_type: z.enum([
      "authorization_code",
      "refresh_token",
...
export const meetingRecurrenceSchema = z
  ...
  .refine(
    (data) => {
      ...
      if (data.type === 3 && !data.monthly_day && !data.monthly_week)
        return false;
      return true;
    },
...
export const createMeetingSchema = z.object({
  topic: z
    .string()
    .min(1)
    .max(200, "Topic must be between 1 and 200 characters"),
...
export const createRegistrantSchema = z.object({
  ...
  first_name: z
    .string()
    .min(1)
    .max(64, "First name must be between 1 and 64 characters"),
  last_name: z
    .string()
    .min(1)
    .max(64, "Last name must be between 1 and 64 characters"),
...
export const userTypeSchema = z
  .enum([1, 2, 3])
  .describe("1=Basic, 2=Licensed, 3=On-prem");
...
export const updateUserSchema = z.object({
  ...
  host_key: z
    .string()
    .regex(/^\d{6,10}$/, "Host key must be 6-10 digits")
    .optional(),
